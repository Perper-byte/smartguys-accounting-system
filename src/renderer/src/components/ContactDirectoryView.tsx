...
export function ContactDirectoryView({
  ...
  const [filterType, setFilterType] = useState<'ALL' | 'PATIENT' | 'DOCTOR' | 'HMO_CORP' | 'SUPPLIER'>('ALL')
  ...
  const filteredContacts = useMemo(() => {
    return contacts.filter((c) => {
      ...
      const matchesType = 
        filterType === 'ALL' || 
        (filterType === 'HMO_CORP' ? (c.type === 'HMO' || c.type === 'CORPORATE') : c.type === filterType)
  ...
  const handleImport = () => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        const contactsSheetName = workbook.SheetNames.find((sheetName) => sheetName.trim().toLowerCase() === 'contacts')
        ...
        if (!result || result.success === false) throw new Error(result?.error || 'Failed to import contacts.')
  ...
  const handleExport = () => {
    ...
    try {
      ...
      worksheet['!cols'] = [
        { wch: 38 }, { wch: 28 }, { wch: 14 }, { wch: 32 }, { wch: 18 },
        { wch: 22 }, { wch: 45 }, { wch: 14 }, { wch: 18 }, { wch: 18 }
      ]
      worksheet['!autofilter'] = { ref: `A1:J${exportData.length + 1}` }

      for (let row = 2; row <= exportData.length + 1; row++) {
        const idCell = worksheet[`A${row}`]; if (idCell) idCell.t = 's';
        const phoneCell = worksheet[`E${row}`]; if (phoneCell) phoneCell.t = 's';
        const tinCell = worksheet[`F${row}`]; if (tinCell) tinCell.t = 's';
        const payableCell = worksheet[`I${row}`]; if (payableCell) { payableCell.t = 'n'; payableCell.z = '₱#,##0.00'; }
        const receivableCell = worksheet[`J${row}`]; if (receivableCell) { receivableCell.t = 'n'; receivableCell.z = '₱#,##0.00'; }
      }
    ...
    <div className="w-full max-w-7xl mx-auto px-6 py-4 flex flex-col font-sans text-gray-800 relative animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        {[
          ...
          <div key={card.type} onClick={() => setFilterType(card.type as any)} className={`p-4 rounded-xl border cursor-pointer transition shadow-sm text-center ${filterType === card.type ? 'bg-[#1B9387] border-[#1B9387] text-white' : 'bg-white border-[#B0DCDA] hover:bg-[#E9FAFA] text-gray-600'}`}>
            <p className={`text-[10px] font-extrabold uppercase tracking-wider mb-1 ${filterType === card.type ? 'text-[#E9FAFA]' : 'text-gray-500'}`}>{card.label}</p>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center gap-4 mb-4">
        <div className="relative w-full lg:w-96">
          ...
          <input type="text" placeholder="Search name, TIN, HMO..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white border border-[#B0DCDA] rounded-md pl-9 pr-4 py-2 text-sm focus:outline-none focus:border-[#1B9387] focus:ring-1 focus:ring-[#1B9387] text-gray-800 shadow-sm" />
        </div>

        <div className="flex items-center space-x-3 w-full lg:w-auto">
          <button onClick={handleImport} className="bg-white border border-[#B0DCDA] hover:bg-[#E9FAFA] text-[#1B9387] px-4 py-2 rounded-md text-sm font-extrabold shadow-sm transition flex items-center space-x-2">
            <span>📥</span><span>Import</span>
          </button>
          <button onClick={handleExport} className="bg-white border border-[#B0DCDA] hover:bg-[#E9FAFA] text-[#1B9387] px-4 py-2 rounded-md text-sm font-extrabold shadow-sm transition flex items-center space-x-2">
            <span>📤</span><span>Export</span>
          </button>

          <div className="relative z-50">
            <button onClick={() => setIsNewContactMenuOpen(!isNewContactMenuOpen)} className="bg-[#1B9387] hover:bg-[#28958B] border border-transparent text-white px-5 py-2 rounded-md text-sm font-extrabold shadow-sm transition flex items-center space-x-2">
            ...
            {isNewContactMenuOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white border border-[#B0DCDA] rounded-lg shadow-xl overflow-hidden py-1 z-50">
                ...
                <button onClick={() => openNewContactModal('CORPORATE')} className="w-full text-left px-4 py-2.5 text-sm font-bold text-gray-700 hover:bg-[#E9FAFA] hover:text-[#1B9387] transition">🏢 Corporate</button>
            ...
            {loading ? (
              ...
              paginatedContacts.map((c) => (
                <React.Fragment key={c.id}>
                  <tr onClick={() => setExpandedContactId(expandedContactId === c.id ? null : c.id)} className={`cursor-pointer transition-colors group ${expandedContactId === c.id ? 'bg-[#E9FAFA]' : 'hover:bg-gray-50 even:bg-gray-50/50 odd:bg-white'}`}>
                    ...
                    <td className="p-4"><span className={`px-2 py-1 rounded-md text-[10px] font-extrabold uppercase tracking-wider border shadow-sm ${getTypeStyle(c.type)}`}>{c.type}</span></td>
                    <td className="p-4 text-xs text-gray-500 font-medium">{c.email ? c.email : c.phone ? c.phone : <span className="italic text-gray-400">Missing info</span>}</td>
                    <td className="p-4 text-center"><span className={`px-2 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider ${c.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-gray-100 text-gray-500 border border-gray-200'}`}>{c.status}</span></td>
                    ...
                    <td className={`p-4 text-center relative ${actionMenuId === c.id ? 'z-30' : 'z-10'}`}>
                      <button onClick={(e) => { e.stopPropagation(); setActionMenuId(actionMenuId === c.id ? null : c.id); }} className="text-gray-400 hover:text-gray-800 px-2 py-1 rounded hover:bg-gray-200 transition text-lg font-bold">⋮</button>
                      {actionMenuId === c.id && (
                        <div className="absolute right-8 top-10 w-40 bg-white border border-[#B0DCDA] rounded-md shadow-xl overflow-hidden py-1 text-left z-20">
                          <button onClick={(e) => { e.stopPropagation(); handleViewDetails(c.id); }} className="w-full text-left px-4 py-2 text-xs font-bold text-gray-700 hover:bg-[#E9FAFA] hover:text-[#1B9387]">👁️ View Details</button>
                          <button onClick={(e) => { e.stopPropagation(); handleEditTin(c); }} className="w-full text-left px-4 py-2 text-xs font-bold text-gray-700 hover:bg-[#E9FAFA] hover:text-[#1B9387]">✏️ Edit TIN</button>
                          <button onClick={(e) => { e.stopPropagation(); handleTransactions(c); }} className="w-full text-left px-4 py-2 text-xs font-bold text-gray-700 hover:bg-[#E9FAFA] hover:text-[#1B9387]">🧾 Transactions</button>
                          <div className="border-t border-gray-100 my-1"></div>
                          <button onClick={(e) => { e.stopPropagation(); handleArchive(c); }} className="w-full text-left px-4 py-2 text-xs font-bold text-red-500 hover:bg-red-50">🗑️ Archive</button>
                        </div>
                      )}
                    </td>
                  </tr>
                      ...
                      <td colSpan={7} className="p-6 border-l-4 border-l-[#1B9387]">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-start">
                          <div className="md:col-span-2">
                            ...
                            {/* 🔥 ALWAYS SHOWS FOR PATIENTS SO YOU KNOW IT WORKS */}
                            {c.type === 'PATIENT' && (
                                <div className="mt-4 pt-4 border-t border-gray-200">
                                  ...
                                  {c.hmo_affiliation ? (
                                      <div className="space-y-1.5">
                                        <p className="text-sm text-gray-800 font-medium"><span className="text-gray-400 mr-2 inline-block w-24 font-bold">Provider:</span> <span className="font-bold">{c.hmo_affiliation}</span></p>
                                        <p className="text-sm text-gray-800 font-medium"><span className="text-gray-400 mr-2 inline-block w-24 font-bold">Card/Policy:</span> <span className="font-mono">{c.hmo_card_no || 'N/A'}</span></p>
                                        {c.hmo_expiry_date && (
                                            <p className="text-sm text-gray-800 font-medium">
                                              <span className="text-gray-400 mr-2 inline-block w-24 font-bold">Expiry Date:</span>
                                              <span className={`font-mono ${new Date(c.hmo_expiry_date) < new Date() ? 'text-red-500 font-bold' : ''}`}>
                                                {new Date(c.hmo_expiry_date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                                      ...
                                      <p className="text-sm text-gray-400 italic">No HMO / Corporate Guarantor assigned.</p>
                                  )}
                                </div>
                            )}
                          </div>
      ...
      {!loading && filteredContacts.length > 0 && (
        <div className="flex flex-col sm:flex-row justify-between items-center text-sm text-gray-500 pt-2">
          ...
          <div className="flex space-x-2">
            <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-4 py-2 border rounded-md text-xs font-bold uppercase tracking-wider transition shadow-sm disabled:opacity-50 disabled:bg-gray-100 disabled:text-gray-400 disabled:border-gray-200 disabled:cursor-not-allowed enabled:bg-[#FBF8F8] enabled:hover:bg-[#E9FAFA] enabled:text-gray-700 enabled:border-[#B0DCDA]">&larr; Prev</button>
            <button onClick={() => setCurrentPage((p) => p + 1)} disabled={currentPage * itemsPerPage >= filteredContacts.length} className="px-4 py-2 border rounded-md text-xs font-bold uppercase tracking-wider transition shadow-sm disabled:opacity-50 disabled:bg-gray-100 disabled:text-gray-400 disabled:border-gray-200 disabled:cursor-not-allowed enabled:bg-[#FBF8F8] enabled:hover:bg-[#E9FAFA] enabled:text-gray-700 enabled:border-[#B0DCDA]">Next &rarr;</button>
          </div>
        </div>
      )}

      <NewContactModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSaveSuccess={() => fetchContacts()} defaultType={newContactType} />
      ...
      {editingTinContact && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-gray-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl w-96 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-4">
              <input type="text" value={newTinValue} onChange={(e) => setNewTinValue(e.target.value)} placeholder="Enter TIN (leave blank to clear)" className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-[#1B9387] focus:ring-1 focus:ring-[#1B9387] shadow-inner" autoFocus onKeyDown={(e) => e.key === 'Enter' && submitEditTin()} />
            </div>
            <div className="p-4 bg-gray-50 flex justify-end space-x-2 border-t border-gray-100">
              <button onClick={() => setEditingTinContact(null)} className="px-4 py-2 text-sm font-bold text-gray-600 hover:bg-gray-200 rounded-md transition">Cancel</button>
              <button onClick={submitEditTin} className="px-4 py-2 text-sm font-bold text-white bg-[#1B9387] hover:bg-[#28958B] rounded-md transition shadow-sm">Save Changes</button>
            </div>
          </div>
        </div>
      )}
