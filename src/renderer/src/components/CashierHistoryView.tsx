...
export function CashierHistoryView({ userId }: { userId: string }) {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      if (!api.getUserSalesHistory) {
        throw new Error('Transaction History API is unavailable.')
      }

      console.log('[Transaction History] Current user:', userId)

      const response = await api.getUserSalesHistory(userId)

      console.log('[Transaction History] Response:', response)

      if (response && response.success === false) {
        throw new Error(response.error || 'Unable to load transactions.')
      }
  ...
  const renderStatusBadge = (status: string) => {
    ...
    if (status === 'PENDING_VOID')
      return (
        <span className="px-2.5 py-1 bg-amber-50 border border-amber-200 text-amber-600 text-[10px] font-extrabold rounded-md uppercase tracking-wider shadow-sm">
          Pending Void
        </span>
      )
    if (status === 'VOIDED')
      return (
        <span className="px-2.5 py-1 bg-red-50 border border-red-200 text-red-500 text-[10px] font-extrabold rounded-md uppercase tracking-wider shadow-sm line-through">
          Voided
        </span>
      )
    return null
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-4 mb-6 border-b border-[#B0DCDA] pb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1 font-medium">
            Review your recent invoices and receipts, or request a void for mistakes.
          </p>
        </div>
                    ...
                    <React.Fragment key={tx.id}>
                      ...
                      >
                        ...
                        <td className="p-4 border-r border-gray-100">
                          ...
                          {tx.billedEntity && tx.billedEntity !== tx.patientName && (
                            <div className="text-[10px] text-gray-400 font-medium mt-1">
                              Billed to: {tx.billedEntity}
                            </div>
                          )}
                        </td>
                          ...
                          <td colSpan={7} className="p-5 border-l-4 border-l-red-400">
                            <div className="flex items-center space-x-4 max-w-3xl mx-auto bg-white p-4 rounded-lg border border-red-200 shadow-sm">
                              ...
                              <div className="flex-1">
                                <p className="text-xs font-extrabold text-red-500 uppercase tracking-wider mb-1">
                                  Request Void for {tx.referenceNo}
                                </p>
                                <input
                                  type="text"
                                  autoFocus
                              ...
                              <div className="flex flex-col space-y-2 mt-4">
                                <button
                                  onClick={() => submitVoidRequest(tx.id)}
                                  className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded text-xs font-bold transition shadow-sm cursor-pointer"
                                >
                                  Submit Request
                                </button>
                                <button
                                  ...
                                  className="bg-white border border-gray-300 text-gray-600 hover:bg-gray-50 px-4 py-2 rounded text-xs font-bold transition shadow-sm cursor-pointer"
                                >
                                  Cancel
                                </button>
                              </div>
      ...
      {!loading && filteredTransactions.length > 0 && (
        <div className="flex flex-col sm:flex-row justify-between items-center mt-4 text-sm text-gray-500">
          <div className="mb-4 sm:mb-0">
            ...
            of <span className="font-bold text-gray-800">{filteredTransactions.length}</span>{' '}
            transactions
          </div>
