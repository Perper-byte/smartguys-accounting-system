...
export function EWTPayoutView({ userId }: { userId: string }) {
    ...
    // ---> NEW: Entity Type State <---
    const [newPayeeType, setNewPayeeType] = useState('DOCTOR'); 
    ...
    useEffect(() => {
        const fetchPayees = async () => {
            try {
                ...
                // ---> STRICT FILTER: Only load Doctors and Suppliers! <---
                const data = await api.getPayees('DOCTOR,SUPPLIER');
                setPayees(data);
    ...
    const handleCreatePayee = async () => {
        ...
        try {
            const api = (window as any).api || (window as any).electronAPI;
            await api.createPayee(newPayeeName, newPayeeType);
            
            const updatedPayees = await api.getPayees('DOCTOR,SUPPLIER');
    ...
    useEffect(() => {
        if (!payeeId || view !== 'NEW') {
            setPayableBalance(0); setAmountToPay(''); return;
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setLoading(true);
            const lines: any[] = [];
            lines.push({ accountId: '2010', debit: grossAmount, credit: 0 }); 
            if (ewtAmount > 0) lines.push({ accountId: '2050', debit: 0, credit: ewtAmount }); 
            lines.push({ accountId: '1010', debit: 0, credit: netAmount }); 

            const selectedPayee = payees.find(p => p.id === payeeId);
            const fullReferenceNo = `${refPrefix}${refSequence.padStart(3, '0')}`;

            const entryData = {
                date: new Date().toISOString(), referenceNo: fullReferenceNo,
            ...
            if (response && response.success === false) return setStatus({ type: 'error', msg: "Database Error: " + response.error });

            setStatus({ type: 'success', msg: `Voucher ${fullReferenceNo} recorded! Net Check Amount: ₱${netAmount.toFixed(2)}.` });
            
            setGenerated2307({ payee: selectedPayee, date: new Date(), gross: grossAmount, tax: ewtAmount, net: netAmount, atc: selectedATC });

            setAmountToPay(''); setRemarks('');
            const newBal = await api.getPayeeBalance(payeeId);
            setPayableBalance(newBal?.payable || 0);
        } catch (error) { setStatus({ type: 'error', msg: "System Error: Could not save payout." }); } finally { setLoading(false); }
    };
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                            ...
                                            <div className="absolute z-20 w-full mt-1 bg-[#202024] border border-[#29292e] rounded-md shadow-2xl overflow-hidden">
                                                <div className="p-2 border-b border-[#29292e] bg-[#121214]">
                                                    <div className="flex justify-between items-center mb-2">
                                                        <span className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">Directory</span>
                                                        <button type="button" onClick={() => setShowAddPayee(!showAddPayee)} className="text-xs font-bold text-[#4f46e5] hover:text-[#5b54f6] transition hover:underline">{showAddPayee ? 'Cancel' : '+ Add New'}</button>
                                                    </div>
                                                    
                                                    {showAddPayee && (
                                                        <div className="mb-3 p-3 bg-[#1a1a1e] border border-[#4f46e5]/50 rounded-md flex gap-2">
                                                            {/* ---> ADDED ENTITY TAGGING SELECTOR <--- */}
                                                            <select value={newPayeeType} onChange={e => setNewPayeeType(e.target.value)} className="bg-[#202024] border border-[#29292e] text-xs text-white rounded px-2 outline-none">
                                                                <option value="DOCTOR">Doctor</option>
                                                                <option value="SUPPLIER">Supplier</option>
                                                            </select>
                                                            <input type="text" placeholder="e.g. Dr. Smith" value={newPayeeName} onChange={e => setNewPayeeName(e.target.value)} className="flex-1 bg-transparent text-sm text-white outline-none" autoFocus />
                                                            <button type="button" onClick={handleCreatePayee} disabled={isSubmittingPayee || !newPayeeName.trim()} className="bg-[#4f46e5] hover:bg-[#5b54f6] text-white text-xs font-bold px-3 py-1.5 rounded">Save</button>
                                                        </div>
                                                    )}
                                                    <input type="text" autoFocus placeholder="🔍 Search database..." value={payeeSearchQuery} onChange={(e) => setPayeeSearchQuery(e.target.value)} className="w-full bg-transparent p-2 text-sm text-white outline-none border border-[#29292e] rounded mt-2" />
                                                </div>
                                                <ul className="max-h-48 overflow-y-auto">
                                                    ...
                                                    {filteredPayees.map(p => (<li key={p.id} onClick={() => { setPayeeId(p.id); setIsPayeeDropdownOpen(false); setPayeeSearchQuery(''); }} className="p-3 text-sm text-white hover:bg-[#4f46e5] cursor-pointer transition border-t border-[#29292e]/50">{p.name} <span className="text-[9px] ml-2 text-gray-500 uppercase">{p.type}</span></li>))}
                                                </ul>
                                            </div>
                                        )}
                                    </div>
                                ...
                                <div className="flex flex-col justify-end">
                                    <div className={`p-4 rounded-md border text-center ${payableBalance > 0 ? 'bg-orange-400/10 border-orange-400/30' : 'bg-[#121214] border-[#29292e]'}`}>
                                        ...
                                        <p className={`text-2xl font-bold font-mono ${payableBalance > 0 ? 'text-orange-400' : 'text-emerald-400'}`}>₱ {payableBalance.toFixed(2)}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-[#121214] p-6 border border-[#29292e] rounded-lg space-y-6">
                                <div className="grid grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <div className="flex"><span className="bg-[#2a2a2f] border border-[#29292e] border-r-0 rounded-l-md px-4 py-3 text-sm font-bold text-gray-400 select-none">{refPrefix}</span><input type="text" required value={refSequence} onChange={e => setRefSequence(e.target.value)} placeholder="001" className="w-full bg-[#121214] border border-[#29292e] rounded-r-md p-3 text-sm font-mono text-white focus:border-[#4f46e5] outline-none transition" /></div>
                                    </div>
                                    <div>
                                        ...
                                        <select value={selectedATC.code} onChange={e => setSelectedATC(ATC_CODES.find(atc => atc.code === e.target.value) || ATC_CODES[3])} className="w-full bg-[#202024] border border-[#29292e] rounded-md p-3 text-sm text-white focus:border-[#4f46e5] outline-none transition cursor-pointer">
                                            {ATC_CODES.map(atc => (<option key={atc.code} value={atc.code}>{atc.code} - {atc.desc}</option>))}
                                        </select>
                                    </div>
                                </div>

                                <div className="grid grid-cols-3 gap-6 items-end pt-4 border-t border-[#29292e]">
                                    <div><label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Gross Payout (A/P)</label><input type="number" required min="0.01" step="0.01" value={amountToPay} onChange={e => setAmountToPay(parseFloat(e.target.value) || '')} placeholder="0.00" className="w-full bg-[#202024] border border-[#29292e] rounded-md p-3 text-lg text-white font-mono focus:border-[#4f46e5] outline-none transition" /></div>
                                    <div><label className="block text-xs font-bold text-yellow-500 uppercase tracking-wider mb-2">Less: {selectedATC.rate}% EWT</label><div className="w-full bg-[#202024] border border-yellow-500/30 rounded-md p-3 text-lg text-yellow-500 font-mono">- ₱ {ewtAmount.toFixed(2)}</div></div>
                                    <div><label className="block text-xs font-bold text-emerald-400 uppercase tracking-wider mb-2">Net Check Amount</label><div className="w-full bg-[#202024] border border-emerald-500/50 rounded-md p-3 text-xl text-emerald-400 font-bold font-mono">₱ {netAmount.toFixed(2)}</div></div>
                                </div>
                            </div>
                                ...
                                <table className="w-full text-left text-sm">
                                    <thead className="bg-[#202024] sticky top-0 z-10 border-b border-[#29292e] shadow-sm">
                                        <tr className="text-[#8d8d99] uppercase tracking-wider text-xs">
                                            <th className="p-4 font-bold">Date</th><th className="p-4 font-bold">Check Ref</th><th className="p-4 font-bold">Payee</th><th className="p-4 font-bold text-right">Gross (A/P)</th><th className="p-4 font-bold text-right text-yellow-500">Tax Withheld</th><th className="p-4 font-bold text-right text-emerald-400">Net Paid</th><th className="p-4 font-bold text-center">Action</th>
                                        </tr>
                                    </thead>
                                                ...
                                                <tr key={idx} className="hover:bg-[#2a2a2f] transition-colors">
                                                    ...
                                                    <td className="p-4 text-center">
                                                        <button onClick={() => {
                                                            ...
                                                            let fallbackATC = ATC_CODES.find(a => a.rate === taxRate) || ATC_CODES[3];
                                                            setGenerated2307({ payee: tx.payee, date: new Date(tx.date), gross: tx.gross, tax: tx.tax, net: tx.net, atc: fallbackATC, isReprint: true, ref: tx.referenceNo });
                                                        }} className="text-xs bg-[#29292e] hover:bg-[#323238] border border-[#3e3e44] px-3 py-1.5 rounded transition text-gray-300 hover:text-white cursor-pointer">🖨️ Re-print</button>
                                                    </td>
                                                </tr>
                                            ))
                                        )}
