...
export function ReconciliationView({ userId }: { userId: string }) {
  ...
  const addTransaction = async (event: React.FormEvent) => {
    ...
    // 🔥 THE FIX: We correctly check for `result.success` instead of `result.id`
    ...
    <div className="max-w-7xl mx-auto h-full flex flex-col text-gray-200">
      ...
      <div className="flex justify-between items-end mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setShowSetup(true)} className="px-4 py-2 bg-[#29292e] hover:bg-[#323238] border border-[#3e3e44] rounded-md text-sm font-bold text-white transition cursor-pointer">
            + Bank Account
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-3 mb-5">
        ...
        <button onClick={loadData} disabled={loading} className="px-4 py-2 bg-[#4f46e5] hover:bg-[#5b54f6] rounded-md text-sm font-bold text-white transition disabled:opacity-50 cursor-pointer">
      ...
      {!accounts.length ? (
        <div className="flex-1 bg-[#202024] border border-[#29292e] rounded-lg p-12 text-center">
          ...
          <button onClick={() => setShowSetup(true)} className="mt-5 px-4 py-2 bg-[#4f46e5] hover:bg-[#5b54f6] rounded-md text-sm font-bold transition cursor-pointer">Set up account</button>
        </div>
      ) : (
        <>
          ...
          <form onSubmit={addTransaction} className="bg-[#202024] border border-[#29292e] rounded-lg p-4 mb-5">
            ...
            <div className="grid grid-cols-[140px_1fr_160px_160px_auto] gap-3">
              ...
              <input type="number" step="0.01" placeholder="Amount (+/-)" value={newTransaction.amount} onChange={event => setNewTransaction({ ...newTransaction, amount: event.target.value })} className="bg-[#121214] border border-[#29292e] rounded-md p-2.5 text-sm text-white focus:border-[#4f46e5] outline-none font-mono" />
              <button className="px-4 bg-[#29292e] hover:bg-[#323238] border border-[#3e3e44] rounded-md text-sm font-bold text-white transition cursor-pointer">Add</button>
            </div>
          </form>
                ...
                {suggestedEntries.map(entry => (
                  <div key={entry.id} className="p-4 border-b border-[#29292e]/70 flex justify-between gap-3">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <button disabled={!selectedTransaction} onClick={() => match(entry.id)} className="mt-2 px-3 py-1 text-[10px] uppercase font-bold rounded bg-[#4f46e5] disabled:bg-[#29292e] disabled:text-gray-600 transition cursor-pointer disabled:cursor-not-allowed">
                        Match
                      </button>
                    </div>
                  </div>
          ...
          <div className="mt-4 text-right">
            {matched.length > 0 && <button onClick={() => matched[0] && unmatch(matched[0].id)} className="text-xs text-gray-500 hover:text-white transition cursor-pointer">Undo most recent match</button>}
          </div>
        </>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-[#202024] border border-[#29292e] rounded-lg shadow-2xl p-6 w-[720px] max-h-[80vh] flex flex-col">
            <div className="flex justify-between items-start mb-4">
              ...
              <button onClick={() => setImportRows([])} className="text-gray-500 hover:text-white text-xl cursor-pointer">×</button>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-5 mt-4 border-t border-[#29292e]">
              <button onClick={() => setImportRows([])} disabled={importing} className="px-4 py-2 bg-[#29292e] hover:bg-[#323238] text-gray-300 rounded text-sm font-bold transition cursor-pointer">Cancel</button>
              <button onClick={confirmImport} disabled={importing || !accountId} className="px-4 py-2 bg-[#4f46e5] hover:bg-[#5b54f6] disabled:opacity-50 text-white rounded text-sm font-bold transition cursor-pointer">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-[#202024] border border-[#29292e] rounded-lg shadow-2xl p-6 w-[420px]">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-[#29292e]">
              <button onClick={() => setTransactionToRemove(null)} className="px-4 py-2 bg-[#29292e] hover:bg-[#3a3a42] text-gray-300 rounded text-sm font-bold transition cursor-pointer">Cancel</button>
              <button onClick={removeTransaction} className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white rounded text-sm font-bold transition cursor-pointer">Remove</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <form onSubmit={createAccount} className="bg-[#202024] border border-[#3e3e44] rounded-lg p-6 w-[440px]">
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button type="button" onClick={() => setShowSetup(false)} className="px-4 py-2 text-sm text-gray-400 hover:text-white transition cursor-pointer">Cancel</button>
              <button type="submit" className="px-4 py-2 bg-[#4f46e5] hover:bg-[#5b54f6] rounded-md text-sm font-bold text-white transition cursor-pointer">Save account</button>
            </div>
          </form>
        </div>
      )}
