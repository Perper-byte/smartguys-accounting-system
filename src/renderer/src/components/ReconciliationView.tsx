...
type BankAccount = {
  ...
  ledger_account_ref?: { name: string }
}
type BankTransaction = {
  ...
  amount: number
  status: string
  matchedEntry?: { reference_no: string } | null
}
type LedgerEntry = {
  id: string
  date: string
...
export function ReconciliationView({ userId }: { userId: string }) {
  ...
  // Array to hold multiple selected ledger entries for One-to-Many matching
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (requestId === loadRequestRef.current)
        setStatus({ type: 'error', message: 'Could not load reconciliation activity.' })
    } finally {
  ...
  useEffect(() => {
    loadAccounts().catch(() =>
      setStatus({ type: 'error', message: 'Could not load bank accounts.' })
    )
  }, [loadAccounts])
  useEffect(() => {
    loadData()
  ...
  // Auto-select if there is a perfect 1-to-1 match to save clicks
  useEffect(() => {
  ...
  const suggestedEntries = useMemo(() => {
    ...
    const score = (entry: LedgerEntry) => {
      const amountScore =
        Math.abs(entry.amount - selectedTransaction.amount) < 0.01
          ? 1000
          : -Math.abs(entry.amount - selectedTransaction.amount)
      const referenceScore = reference && normalize(entry.referenceNo).includes(reference) ? 200 : 0
      const descriptionScore = description
        ? description
            .split(/\s+/)
            .filter((word) => word.length > 3 && normalize(entry.description).includes(word))
            .length * 10
        : 0
      const days =
        Math.abs(
          new Date(entry.date).getTime() - new Date(selectedTransaction.transaction_date).getTime()
        ) / 86400000
  ...
  // Dynamic Math for the Variance Counter
  ...
  const createAccount = async (event: React.FormEvent) => {
    ...
    if (!result?.success)
      return setStatus({
        type: 'error',
        message: result?.error || 'Could not create bank account.'
      })
  ...
  const addTransaction = async (event: React.FormEvent) => {
    ...
    if (!accountId || !newTransaction.description || !Number(newTransaction.amount)) {
      return setStatus({
        type: 'error',
        message: 'Date, description, and a non-zero amount are required.'
      })
    }
    const result = await api.createBankTransaction({
      ...newTransaction,
  ...
  const unmatch = async (transactionId: string) => {
    ...
    if (!result?.success)
      return setStatus({ type: 'error', message: result?.error || 'Could not undo match.' })
  ...
  const removeTransaction = async () => {
    ...
    if (!result?.success)
      return setStatus({
        type: 'error',
        message: result?.error || 'Could not remove bank transaction.'
      })
  ...
  const handleImportFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (headerRow < 0)
        throw new Error(
          'Could not find a header row. Expected Date, Description, and Amount columns.'
        )
      ...
      const referenceColumn = findColumn([
        'reference',
        'referenceno',
        'transactionid',
        'checkno',
      ...
      if (
        ...
        throw new Error(
          'Required columns are missing. Include Date, Description, and Amount, or Debit/Credit columns.'
        )
      ...
      const parsedRows = rows
        ...
        .map((row) => {
          ...
          return {
            date: parseImportDate(row[dateColumn]),
            description: String(row[descriptionColumn] ?? '').trim(),
            referenceNo: referenceColumn >= 0 ? String(row[referenceColumn] ?? '').trim() : '',
            amount
          }
        })
        .filter((row) => row.date || row.description || row.amount)
      const invalidRows = parsedRows.filter(
        (row) => !row.date || !row.description || !Number.isFinite(row.amount) || row.amount === 0
      )
      if (invalidRows.length)
        throw new Error(
          `${invalidRows.length} row(s) have an invalid date, description, or non-zero amount.`
        )
      ...
      setStatus({
        type: 'error',
        message: error.message || 'Could not read the bank statement file.'
      })
    }
  }
  ...
  const confirmImport = async () => {
    ...
    setImporting(true)
    const result = await api.importBankTransactions({
      bankAccountId: accountId,
      userId,
    ...
    setStatus({
      type: 'success',
      message: `${result.count} bank transaction(s) imported successfully.${skippedMessage}`
    })
    setImporting(false)
    await loadData()
  }
        ...
        <div className="flex justify-between items-end mb-6 border-b border-[#B0DCDA] pb-4 shrink-0">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1 font-medium">
              Match bank activity to posted ledger entries and keep the cash balance explainable.
            </p>
          </div>
          ...
          <div className="flex flex-col flex-1 min-h-0 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-4 gap-4 mb-6 shrink-0">
              <Metric
                ...
                detail={money(unmatchedBankTotal)}
              ...
              <Metric
                ...
                detail={`${transactions.length ? Math.round((matched.length / transactions.length) * 100) : 0}% of activity`}
                tone="text-[#1B9387]"
              />
              ...
              <div className="bg-[#FBF8F8] border border-[#B0DCDA] rounded-xl p-4 shadow-sm flex flex-col justify-between">
                <div className="flex justify-between items-start">
                  ...
                  >
                    {Number(statementOpening) && Number(statementClosing)
                      ? money(
                          Number(statementClosing) -
                            Number(statementOpening) -
                ...
                <div className="flex gap-2 mt-3">
                  <input
                    ...
                    value={statementOpening}
                    onChange={(event) => setStatementOpening(event.target.value)}
                    className="w-1/2 bg-white border border-gray-200 rounded px-2 py-1.5 text-xs font-mono font-bold text-gray-800 focus:border-[#1B9387] outline-none shadow-inner placeholder:text-gray-400 placeholder:font-sans placeholder:font-normal"
                  />
                  <input
                    type="number"
                    step="0.01"
                    ...
                    onChange={(event) => setStatementClosing(event.target.value)}
                    className="w-1/2 bg-white border border-gray-200 rounded px-2 py-1.5 text-xs font-mono font-bold text-gray-800 focus:border-[#1B9387] outline-none shadow-inner placeholder:text-gray-400 placeholder:font-sans placeholder:font-normal"
                  />
                </div>
              </div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex justify-between items-center">
                            <span
                              className={`text-xs font-bold ${isSelected ? 'text-[#1B9387]' : 'text-gray-500'}`}
                            >
                            ...
                            <span
                              className={`font-mono font-black tabular-nums ${transaction.amount >= 0 ? 'text-emerald-600' : 'text-rose-500'}`}
                            >
                          ...
                          <div className="flex items-center justify-between mt-3 pt-2 border-t border-gray-100/50">
                            ...
                            >
                              {isMatched
                                ? `✓ Matched to ${transaction.matchedEntry?.reference_no || 'entry'}`
                                : isSelected
                                  ? '▶ Matching...'
                                  : 'Click to match ➔'}
                            </p>
            ...
            <div className="bg-white border border-[#B0DCDA] rounded-xl shadow-2xl p-0 w-[720px] max-h-[85vh] flex flex-col animate-in zoom-in-95 duration-200 overflow-hidden">
              <div className="px-6 py-5 border-b border-[#B0DCDA] bg-[#FBF8F8] flex justify-between items-start shrink-0">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1 font-medium">
                    ...
                    {importRows.length} transaction(s)
                  </p>
                </div>
                <button
              ...
              <div className="overflow-auto bg-gray-50/30 p-6">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {importRows.slice(0, 100).map((row, index) => (
                        <tr key={`${row.date}-${index}`} className="hover:bg-gray-50 transition">
                          ...
                          <td
                            className={`p-3 text-right font-mono font-black tabular-nums ${row.amount >= 0 ? 'text-emerald-600' : 'text-rose-500'}`}
                          >
                            {row.amount >= 0 ? '+' : '-'}
                            {money(row.amount)}
                          </td>
                ...
                {importRows.length > 100 && (
                  <p className="mt-4 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">
                    Showing the first 100 rows. All {importRows.length} rows will be imported.
                  </p>
                )}
              </div>
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-end gap-3 shrink-0">
                ...
                <button
                  ...
                  className="px-6 py-2.5 bg-[#1B9387] hover:bg-[#28958B] disabled:opacity-50 disabled:bg-gray-400 text-white rounded-md text-sm font-bold uppercase tracking-wider transition cursor-pointer shadow-sm flex items-center gap-2"
                >
                  {importing ? (
                    <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm print:hidden">
            <div className="bg-white border border-[#B0DCDA] rounded-xl shadow-2xl p-8 w-[420px] animate-in zoom-in-95 duration-200">
              ...
              <p className="text-sm text-gray-600 mb-8 font-medium leading-relaxed">
                Remove <strong className="text-gray-800">{transactionToRemove.description}</strong>{' '}
                from the active reconciliation queue? The record will remain available in the audit
                history.
              </p>
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-md text-sm font-bold transition cursor-pointer shadow-sm uppercase tracking-wider"
                >
                  Remove
                </button>
              </div>
        ...
        {showSetup && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm print:hidden">
            <form
              ...
              className="bg-white border border-[#B0DCDA] rounded-xl overflow-hidden shadow-2xl w-[440px] animate-in zoom-in-95 duration-200 flex flex-col"
            >
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#FBF8F8] border border-gray-200 rounded-md p-2.5 text-sm font-bold text-gray-800 focus:border-[#1B9387] outline-none transition focus:bg-white"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-extrabold text-gray-500 uppercase tracking-wider mb-1.5">
                  ...
                  <input
                    ...
                    onChange={(event) => setSetup({ ...setup, accountNumber: event.target.value })}
                    className="w-full bg-[#FBF8F8] border border-gray-200 rounded-md p-2.5 text-sm font-mono font-bold text-gray-800 focus:border-[#1B9387] outline-none transition focus:bg-white"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-extrabold text-gray-500 uppercase tracking-wider mb-1.5">
                  ...
                  <select
                    required
                    value={setup.ledgerAccount}
                    onChange={(event) => setSetup({ ...setup, ledgerAccount: event.target.value })}
                    className="w-full bg-[#FBF8F8] border border-gray-200 rounded-md p-2.5 text-sm font-bold text-gray-800 focus:border-[#1B9387] outline-none transition focus:bg-white"
                  >
