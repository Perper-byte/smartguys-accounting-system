...
export function POSBillingView({ userId }: { userId: string }) {
  ...
  const loadInitialData = async () => {
    try {
      ...
      if (!api) {
        console.error('Electron API is unavailable.')
        return
      }
      /*
    ...
    setItems(
      items.map((item) => {
        if (item.id === id) {
          ...
          /*
           * Reset description when changing category
           */
  ...
  const calculateLabDropdownPosition = (element: HTMLInputElement) => {
    ...
    /*
     * Keep dropdown inside right edge.
     */
    ...
    /*
     * Normally show directly underneath
     * the description input.
     */
    let top = rect.bottom + 6

    /*
     * Only open upward if the input is
     * genuinely near the bottom.
     */
    const spaceBelow = window.innerHeight - rect.bottom

    if (spaceBelow < 180 && rect.top > dropdownHeight) {
  ...
  useEffect(() => {
    const syncLabDropdown = () => {
      if (activeLabRow === null || !labInputRef.current) {
        return
      }

      const element = labInputRef.current

      const rect = element.getBoundingClientRect()

      /*
       * If the input itself is no longer
       * visible, close the dropdown.
       */
      ...
      /*
       * Otherwise follow the input while
       * the page/table is scrolling.
       */
      setLabDropdownPosition(calculateLabDropdownPosition(element))
    }

    /*
     * true = also detect scrolling inside
     * nested containers.
     */
  ...
  const isPaid = paymentMethod ? (paymentMethod === 'CASH' ? tendered >= grandTotal : true) : false
  // ============================================================
  // PATIENT FILTER
  // ============================================================
  ...
  const filteredPayees = payees.filter((p) => {
    const matchesSearch = p.name?.toLowerCase().includes(payeeSearchQuery.toLowerCase())
    if (!matchesSearch) {
      return false
    }
    /*
     * HMO should only display HMO entities.
     */
    if (paymentMethod === 'HMO') {
      return p.type === 'HMO'
    }
    /*
     * Charge should use Corporate entities.
     */
  ...
  const handleCheckoutClick = (e: React.FormEvent) => {
    ...
    if (items.length === 0 || grandTotal === 0) {
      return alert('Please add items to bill.')
    }
    if (paymentMethod === 'CASH' && tendered < grandTotal) {
      return alert('Amount tendered is insufficient.')
    }
    if (isSCPWD && !scPwdId.trim()) {
      return alert('SC/PWD ID is required.')
    }
    if ((paymentMethod === 'HMO' || paymentMethod === 'CHARGE') && !payeeId) {
      return alert('A Guarantor must be selected.')
    }
    if (paymentMethod === 'HMO' && !loaNumber.trim()) {
      return alert('LOA Number is required.')
    }
  ...
  const handleConfirmSubmit = async () => {
    try {
      ...
      /*
       * If HMO/Charge exists,
       * journal attaches to guarantor.
       *
       * Otherwise patient.
       */
      ...
      const description =
        ...
        ` | Billed To: ${payeeId ? selectedPayeeName : 'Self'}` +
        ` (${paymentMethod})` +
        `${paymentMethod === 'HMO' ? ` LOA: ${loaNumber}` : ''}` +
        `${paymentMethod === 'GCASH' ? ` Ref: ${referenceNo}` : ''}`
      const finalReferenceNo = manualInvoiceNo.trim()
      ...
      <div className="w-full flex justify-center items-center min-h-[calc(100vh-64px)] bg-[#f9fafb] animate-in zoom-in-95 duration-300 p-8">
        <div className="bg-white p-12 rounded-3xl shadow-xl border border-[#B0DCDA] flex flex-col items-center max-w-lg w-full text-center">
          ...
          <div className="text-6xl font-black font-mono tracking-tighter text-[#1B9387] mb-8">
            ...
            {successData.total.toLocaleString('en-US', {
          ...
          <div className="w-full space-y-4 text-base border-t border-b border-gray-100 py-8 mb-8 text-left">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-bold text-gray-900 uppercase">{successData.method}</span>
            </div>
            {successData.method === 'CASH' && (
              <div className="flex justify-between">
                ...
                <span className="font-bold font-mono text-emerald-600">
                  ₱{' '}
                  {successData.change.toLocaleString('en-US', {
                    minimumFractionDigits: 2
                  })}
                </span>
              </div>
    ...
    <div className="w-full min-h-[calc(100vh-64px)] flex justify-center items-start bg-[#f9fafb] p-6 lg:p-10">
      <div className="w-full max-w-[1600px] flex flex-col text-gray-800 font-sans relative animate-in fade-in duration-300">
        ...
        <div className="mb-8 border-b border-[#B0DCDA] pb-6 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-base text-slate-500 font-medium mt-1">
              Generate EOPT-Compliant invoices and process transactions.
            </p>
          </div>
        </div>
        <form
            ...
            <div className="bg-white rounded-2xl border border-[#B0DCDA] shadow-sm overflow-hidden">
              ...
              <div className="p-8 space-y-8">
                      ...
                      className={`
                        ...
                        ${
                          isPatientDropdownOpen
                            ? 'border-[#1B9387] ring-2 ring-[#E9FAFA]'
                ...
                <div className="border border-gray-200 rounded-xl overflow-hidden transition-all bg-gray-50/50">
                  <div
                    className="px-5 py-4 flex items-center gap-4 cursor-pointer hover:bg-gray-100 transition-colors"
            ...
            <div className="bg-white rounded-2xl border border-[#B0DCDA] shadow-sm flex flex-col overflow-hidden min-h-[300px]">
              <div className="px-8 py-5 border-b border-gray-100 bg-[#FBF8F8] flex justify-between items-center">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-white border border-[#B0DCDA] text-gray-600 rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-[#E9FAFA] hover:text-[#1B9387] transition-colors shadow-sm"
                >
              ...
              <div className="flex-1 overflow-auto">
                <table className="w-full text-left border-collapse min-w-[900px]">
                  <thead className="bg-white sticky top-0 shadow-sm z-10">
                    <tr>
                      <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider border-b border-gray-200 w-64">
                        Category
                      </th>
                      <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider border-b border-gray-200">
                        Description
                      </th>
                      <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider border-b border-gray-200 text-center">
                        VAT
                      </th>
                      <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider border-b border-gray-200 text-center">
                        Qty
                      </th>
                      <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider border-b border-gray-200 text-right">
                        Unit Price
                      </th>
                      <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider border-b border-gray-200 text-right">
                        Total
                      ...
                      <tr key={item.id} className="hover:bg-gray-50 transition-colors group">
                        ...
                        <td className="px-6 py-3 min-w-[250px]">
                          <input
                            ...
                            className={`
                              ...
                              focus:outline-none
                              focus:ring-2
                              focus:ring-[#1B9387]/30
                              rounded-lg
                              px-3
                              py-2
                              ${
                                item.accountCode === '4020'
                                  ? 'text-[#1B9387] placeholder-[#1B9387]/50'
                                  : 'text-gray-900'
                              }
                            `}
                                    ...
                                    .map((test, index) => (
                                      ...
                                      >
                                        <div className="min-w-0">
                                          <span className="font-bold text-base block truncate group-hover:text-[#1B9387]">
                                            {test.name}
                                          </span>
                                          <span className="block text-xs text-gray-400 uppercase font-bold mt-1 truncate">
                                            {test.category}
                                          </span>
                                        </div>
                                        {Number(test.price) > 0 && (
                                          <span className="font-mono text-[#1B9387] font-bold text-base whitespace-nowrap">
                                            ₱
                        ...
                        <td className="px-6 py-3 text-center">
                          <input
                            ...
                            title="Is this item subject to VAT?"
                          />
                        </td>
                        {/* QUANTITY */}
                        <td className="px-6 py-3 w-32">
                          <div className="flex items-center justify-between border border-gray-300 rounded-lg overflow-hidden bg-white hover:border-[#1B9387] transition-colors focus-within:ring-2 focus-within:ring-[#E9FAFA] focus-within:border-[#1B9387]">
                            <button
                              ...
                              className="px-3 py-2 bg-gray-50 hover:bg-gray-200 text-gray-600 font-bold border-r border-gray-200 transition-colors"
                            >
                            ...
                            <input
                              ...
                              min="1"
                              required
                              className="w-10 text-center bg-transparent text-base font-bold text-gray-900 focus:outline-none appearance-none"
                              value={item.quantity || ''}
                              onChange={(e) =>
                            ...
                            <button
                              type="button"
                              onClick={() => updateItemQty(item.id, 1)}
                              className="px-3 py-2 bg-gray-50 hover:bg-gray-200 text-gray-600 font-bold border-l border-gray-200 transition-colors"
                            >
                              <Plus className="w-4 h-4" />
                            </button>
                          </div>
                        ...
                        <td className="px-6 py-3 w-36">
                          <input
                            ...
                            step="0.01"
                            required
                            className="w-full text-right bg-transparent text-base font-bold font-mono text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#1B9387]/30 rounded-lg px-3 py-2"
                        ...
                        <td className="px-6 py-3 text-center w-16">
                          <button
                            ...
                            className="p-2 bg-rose-50 text-rose-500 border border-rose-100 hover:bg-rose-500 hover:text-white hover:border-rose-500 rounded-lg transition-all shadow-sm focus:outline-none focus:ring-2 focus:ring-rose-200"
                            title="Remove Item"
                          >
                ...
                <div className="flex-[1.5] flex flex-col space-y-8">
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      {[
                          ...
                          className={`
                            ...
                            ${
                              paymentMethod === method.id
                                ? 'bg-[#E9FAFA] border-[#1B9387] text-[#1B9387] shadow-sm ring-1 ring-[#1B9387]'
                        ...
                        >
                          <method.icon
                            className={`w-6 h-6 ${
                              paymentMethod === method.id ? 'text-[#1B9387]' : 'text-gray-400'
                            }`}
                          />
                          {method.label}
                        </button>
                  ...
                  <div className="min-h-[100px]">
                    ...
                    {paymentMethod === 'CASH' && (
                      <div className="grid grid-cols-2 gap-5 animate-in fade-in slide-in-from-bottom-2">
                        <div>
                          ...
                          {amountTendered !== '' && tendered < grandTotal && (
                            <p className="text-xs font-bold text-red-500 uppercase mt-2">
                              Insufficient Amount
                            </p>
                          )}
                        </div>
                    ...
                    {paymentMethod === 'GCASH' && (
                      <div className="animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <input
                          ...
                          className="w-full px-5 py-4 bg-blue-50 border border-blue-200 text-blue-900 rounded-xl text-base font-mono focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 shadow-sm"
                          value={referenceNo}
                    ...
                    {(paymentMethod === 'HMO' || paymentMethod === 'CHARGE') && (
                      <div className="grid grid-cols-2 gap-5 animate-in fade-in slide-in-from-bottom-2 relative z-50">
                        {/* GUARANTOR / PROVIDER */}
                        <div className="relative z-[100]">
                          <label className="block text-xs font-bold text-indigo-500 uppercase tracking-wider mb-2">
                            {paymentMethod === 'HMO'
                              ? 'HMO / Provider'
                              : 'Guarantor / Corporate Account'}
                          </label>
        ...
        `}
                          ...
                          {/* =========================================
          DROPDOWN — anchored to this field
      ========================================= */}
          ...
          "
                ...
                "
                              ...
                              <button
                                ...
                                onMouseDown={(e) => {
                                  ...
                                  setModalDefaultType(paymentMethod === 'HMO' ? 'HMO' : 'CORPORATE')
            ...
            "
                              >
                                + Add New {paymentMethod === 'HMO' ? 'HMO' : 'Corporate Account'}
                              </button>
          ...
          "
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        <Receipt className="w-6 h-6 mr-1" />
                        PROCESS PAYMENT — ₱{' '}
                        {grandTotal.toLocaleString('en-US', {
                          minimumFractionDigits: 2
                        })}
          ...
          <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="bg-white border border-[#B0DCDA] rounded-3xl shadow-2xl p-10 w-[550px] max-h-[85vh] overflow-y-auto animate-in zoom-in-95 duration-200">
              <h3 className="text-2xl font-extrabold text-gray-800 mb-6 border-b border-gray-100 pb-4 flex items-center gap-3">
                <CheckCircle className="w-7 h-7 text-[#1B9387]" />
                Confirm Payment
              </h3>
              <div className="space-y-6 text-base">
                <div className="grid grid-cols-2 gap-y-4 pb-5 border-b border-gray-100">
                  ...
                  <span
                    className={`font-bold text-right uppercase ${
                      paymentMethod === 'CHARGE' ? 'text-rose-500' : 'text-[#1B9387]'
                    }`}
                  >
                    {paymentMethod}
                  </span>
                  {(paymentMethod === 'HMO' || paymentMethod === 'CHARGE') && (
                    <>
                      ...
                      <span className="text-indigo-700 font-bold text-right mt-3">
                        {selectedPayeeName}
                      </span>
                    </>
                  )}
                  ...
                  {paymentMethod === 'GCASH' && (
                    <>
                      ...
                      <span className="text-blue-600 font-mono font-bold text-right">
                        {referenceNo}
                      </span>
                    </>
                  )}
