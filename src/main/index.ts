...
app.whenReady().then(() => {
  ...
  ipcMain.handle('toggle-employee-status', async (e, id, isActive) => { try { return await PayrollService.toggleEmployeeStatus(id, isActive); } catch (err: any) { return { success: false, error: err.message }; } });
  ...
  ipcMain.handle('create-user', async (e, userData) => { 
      try { 
          if (typeof UserService.createUser !== 'function') throw new Error("Backend function missing");
  ...
  ipcMain.handle('get-invoice-tracker', async () => { try { return await ReportsService.getInvoiceTracker(); } catch (err: any) { return []; } });
  ...
  ipcMain.handle('get-contacts-with-balances', async () => { 
      try { 
          // Safely check if the reverted LedgerService actually has this function
          if (typeof LedgerService.getContactsWithBalances === 'function') {
              return await LedgerService.getContactsWithBalances(); 
          }
  ...
  // Bank Accounts safe fallback
  ipcMain.handle('get-bank-accounts', async () => { 
      try { 
          if (typeof LedgerService.getBankAccounts === 'function') {
              return await LedgerService.getBankAccounts(); 
          }
          return []; // Return empty array so the frontend doesn't crash
      } catch (err) { 
          return []; 
  ...
  ipcMain.handle('create-bank-transaction', async (e, data) => { 
      try { return typeof LedgerService.createBankTransaction === 'function' ? await LedgerService.createBankTransaction(data) : { error: "Backend function missing" }; } 
      catch (err: any) { return { error: err.message }; } 
  });

  ipcMain.handle('import-bank-transactions', async (e, data) => { 
      try { return typeof LedgerService.importBankTransactions === 'function' ? await LedgerService.importBankTransactions(data) : { success: false, error: "Backend function missing" }; } 
      catch (err: any) { return { success: false, error: err.message }; } 
  });

  ipcMain.handle('match-bank-transaction', async (e, bankTxId, journalId, userId) => { 
      try { return typeof LedgerService.matchBankTransaction === 'function' ? await LedgerService.matchBankTransaction(bankTxId, journalId, userId) : { success: false, error: "Backend function missing" }; } 
      catch (err: any) { return { success: false, error: err.message }; } 
  });

  ipcMain.handle('unmatch-bank-transaction', async (e, bankTxId) => { 
      try { return typeof LedgerService.unmatchBankTransaction === 'function' ? await LedgerService.unmatchBankTransaction(bankTxId) : { success: false, error: "Backend function missing" }; } 
      catch (err: any) { return { success: false, error: err.message }; } 
  });

  ipcMain.handle('remove-bank-transaction', async (e, bankTxId, userId) => { 
      try { return typeof LedgerService.removeBankTransaction === 'function' ? await LedgerService.removeBankTransaction(bankTxId, userId) : { success: false, error: "Backend function missing" }; } 
      catch (err: any) { return { success: false, error: err.message }; } 
  });

 ipcMain.handle('create-bank-account', async (e, data) => { 
      try { 
          if (typeof LedgerService.createBankAccount === 'function') {
              return await LedgerService.createBankAccount(data); 
          }
          return { success: false, error: "Backend function 'createBankAccount' is missing." }; 
      } catch (err: any) { 
          return { success: false, error: err.message }; 
      } 
  });
  ipcMain.handle('get-reconciliation-data', async (e, id, start, end) => { 
      try { return typeof LedgerService.getReconciliationData === 'function' ? await LedgerService.getReconciliationData(id, start, end) : { transactions: [], entries: [] }; } catch (err) { return { transactions: [], entries: [] }; } 
  });
});
