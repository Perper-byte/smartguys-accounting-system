...
app.whenReady().then(() => {
    ...
    ipcMain.handle('get-payees', async (e, typeFilter) => {
        try {
            return await LedgerService.getPayees(typeFilter)
        } catch (err) {
            return []
        }
    })
    ...
    ipcMain.handle('ledger:submitEntry', async (e, entryData) => {
        try {
            ...
            await AuditService.logAction(entryData.userId || 'SYSTEM', "CREATED TRANSACTION", `Posted ref ${entryData.referenceNo}`);
            return result;
        } catch (err: any) {
    ...
    setInterval(async () => {
        ...
        try {
            ...
            console.log(`🔄 [SYNC] Connection restored! Pushing ${queue.length} offline transactions to Ubuntu Server...`);
            ...
            for (const entryData of queue) {
                try {
                    ...
                    await AuditService.logAction(entryData.userId || 'SYSTEM', "SYNCED OFFLINE TRANSACTION", `Auto-synced ref ${entryData.referenceNo} from local cache`);
