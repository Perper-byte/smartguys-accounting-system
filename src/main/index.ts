...
import { AuditService } from './services/audit.service'; // ---> NEW IMPORT
...
app.whenReady().then(() => {
  ...
  // LEDGER
  ipcMain.handle('get-payees', async () => { try { return await LedgerService.getPayees(); } catch (err) { return []; } });
  ipcMain.handle('create-payee', async (e, name: string) => { return await LedgerService.createPayee(name); });
  ...
  ipcMain.handle(IPC_CHANNELS.LEDGER.SUBMIT_ENTRY, async (e, entryData) => { 
      try { 
          ...
          // ---> AUTO-LOG: Created a Transaction <---
  ...
  ipcMain.handle('get-all-recent-transactions', async () => { try { return await LedgerService.getAllRecentTransactions(); } catch (err) { return []; } });
  ...
  ipcMain.handle('approve-void', async (e, id, managerId) => { 
      try { 
          ...
          // ---> AUTO-LOG: Manager Approved a Void <---
