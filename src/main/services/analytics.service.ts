...
export class AnalyticsService {
    static async getDashboardMetrics(timeframe: 'daily' | 'weekly' | 'monthly' | 'quarterly' | 'yearly' = 'monthly') {
        try {
            ...
            // Sort biggest expenses first for the Donut Chart
            expenseBreakdown.sort((a, b) => b.value - a.value);

            // 3. CALCULATE LIQUIDITY (Total All-Time Cash, including Petty Cash 1020)
            ...
            // Return the exact structure the UI is expecting
            return {
                kpi: { 
